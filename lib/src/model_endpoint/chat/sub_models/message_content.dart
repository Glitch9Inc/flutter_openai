...
class MessageContent {
  ...
  String toString() => switch (type) {
        ...
        'image_base64' =>
          'MessageContent(type: $type, imageBase64: $imageBase64)',
...
class MessageText {
  ...
  const MessageText(
      {required this.value, required this.annotations, this.isString = false});
  ...
  factory MessageText.fromMap(map) {
    ...
    if (map is Map<String, dynamic>) {
      return MessageText(
        value: MapSetter.set<String>(map, 'value'),
  ...
  toJson() {
    if (isString) {
      ...
      return {
        'value': value,
        'annotations':
            annotations!.map((annotation) => annotation.toMap()).toList(),
      };
    }
  }
}
...
class FilePathAnnotation extends Annotation {
  ...
  FilePathAnnotation({
    ...
    required int endIndex,
            ...
            type: type, text: text, startIndex: startIndex, endIndex: endIndex);
