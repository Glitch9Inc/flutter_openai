...
import 'package:flutter_openai/src/requests/interfaces/run_interface.dart';
...
interface class RunRequest implements RunInterface {
  ...
  @override
  Future<Run?> retrieve(String threadId, String runId, {Client? client}) async {
    String ep = '/threads/{thread_id}/runs/{run_id}';
    final formattedEndpoint = ep.replaceAll("{thread_id}", threadId).replaceAll("{run_id}", runId);

    var resultRun =
        await RequestUtils.retrieve(formattedEndpoint, (p0) => Run.fromMap(p0), isBeta: true);
